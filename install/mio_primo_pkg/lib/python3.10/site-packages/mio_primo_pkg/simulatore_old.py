from .kalman_filter import *   #ho aggiunto il punto
import numpy as np
...
#Costanti chhe definiscono il comportamento della simulazione
#Ci saranno 90 frame al secondo, 40 landmark distribuiti in un'area quadrata di lato 30 centrata nell'origine
#La freccia che indica l'orientamento del robot avrà una lunghezza di 0.5 unità
...
# ----------------------------
class Unicycle:
    #la classe init inizializza lo stato del robot unicycle, i comandi di velocità lineare e angolare, i guadagni proporzionali per il controllo
    ...
    # Equazioni differenziali del modello unicycle
    ...
    # Avanzamento del modello unicycle di un passo temporale
...
# ----------------------------
# Planar IMU model
# simula un sensore propriocettivo, fondamentale per l'EKF, simula un accelerometro e giroscopio con rumore e bias
# non restituisce la velocità vera, ma una misura sporca, Noise-> aggiunge rumore casuale gaussiano (vibrazioni, distrurbi elettrici)
# Bias (Deriva) -> aggiunge un errore costante che cambia lentamente nel tempo. Questo rende difficile il 
# compito dell' EKF, che deve stimare non solo la posizione ma anche il bias.
# step(): calcola l'accelerazione nel sistema di riferimento del robor e aggiunge disturbi
# ----------------------------
class IMU:
    """
    Planar IMU:
...
#Questo siula la futura telecmera con YOLO
# check:fov verifica se un landmark è visibile e controlla:
# - la distanza del landmark dal robot è compresa tra min e max range
# - l'angolo del landmark rispetto alla direzione del robot è compreso nel campo visivo
# project: proietta la posizione del landmark nel sistema di riferimento della telecamera e
# calcola la coordinata pixel (1D) del landmark (nel mio progetto dovrà essere 2D)
...
# ----------------------------
# Simulation with EKF
# E' il Main loop e mette tutto insieme e gestisce la grafica
# ----------------------------
